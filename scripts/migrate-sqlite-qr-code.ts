 ...
 *
 * Usage:
 *   npx ts-node scripts/migrate-sqlite-qr-code.ts
 *
...
try {
  ...
  const tableInfo = db.prepare("PRAGMA table_info(links)").all() as any[]
  const hasQrCodeColumn = tableInfo.some((col: any) => col.name === 'qr_code')
  ...
  const indexInfo = db.prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_links_qr_code'").get() as any
